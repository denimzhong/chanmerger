...
func (g *Generator) generate(typeName string) {
	...
	g.Printf("\n")
	g.Printf(stringMergeRecursion, typeName, fname)
	g.Printf("\n\n")
	g.Printf(stringMergeTwo, typeName, fname)
}
...
var stringMergeRecursion = `func merge%[2]sChan(in ...chan %[1]s) chan %[1]s {
	emptyChanFn := func() chan %[1]s {empty:=make(chan %[1]s);close(empty);return empty;}
	len:=len(in)
	switch len {
	case 0:
		return emptyChanFn()
...
var stringMergeTwo = `func mergeTwo%[2]sChan(ch1, ch2 chan %[1]s) chan %[1]s {
	merge := make(chan %[1]s)
	go func() {
		defer close(merge)
		flag := 0
