...
func main() {
	...
	reg := regexp.MustCompile(chanExpr)
	for _, typeName := range types {
		if !reg.MatchString(typeName) {
			log.Fatalf("error: invalid chan type: %s", typeName)
		}
		g.generate(typeName)
	}
	...
	if len(output) == 0 {
		...
		baseName := fmt.Sprintf("%s_merge.go", strings.Join(ss, "_"))
...
func (g *Generator) generate(typeName string) {
	fnName := convertFunctionName(typeName)

	g.Printf("\n")
	g.Printf(stringMergeRecursion, typeName, fnName)
	g.Printf("\n\n")
	g.Printf(stringMergeTwo, typeName, fnName)
}
...
func convertFunctionName(typeName string) string {
	ss := strings.Split(typeName, " ")
	if len(ss) < 2 {
		log.Fatalf("error: invalid channel type naming: %s", typeName)
	}
	return stringFirstUppper(ss[1]) + stringFirstUppper(ss[0])
}
...
var stringMergeRecursion = `func merge%[2]s(in ...%[1]s) %[1]s {
	emptyChanFn := func() %[1]s {empty:=make(%[1]s);close(empty);return empty;}
	len:=len(in)
	switch len {
	case 0:
		return emptyChanFn()
	case 1:
		return in[0]
	case 2:
		return mergeTwo%[2]s(in[0], in[1])
	default:
		left := merge%[2]s(in[:len/2]...)
		right := merge%[2]s(in[len/2:]...)
		return merge%[2]s(left, right)
	}
}`

var stringMergeTwo = `func mergeTwo%[2]s(ch1, ch2 %[1]s) %[1]s {
	merge := make(%[1]s)
	go func() {
		defer close(merge)
		flag := 0
